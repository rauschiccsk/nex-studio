...
| **`.env.example` parse + compose union (CR-026)** | CR-022 §C-1 row sľuboval *"parse-uje `<source>/.env.example` + `services.backend.environment`"* ale implementácia parsovala len compose.environment. nex-inbox má secrets (`LAUNCH_TOKEN`, `JWT_SECRET_KEY`, `EMAIL_CREDS_ENCRYPTION_KEY`) iba v `.env.example` (compose má len DB connection + tenant overrides) → UAT generated `.env` ich vynechal → pydantic Settings vyhodil `Field required` (Bug #11 zachytený smoke testom 2026-05-26). Fix: `_uat_lib.detect_env_example(source)` parse .env.example do dict (rovnaký parser ako `read_uat_env`, ale point at `<source>/.env.example`). `detect_backend_env_vars` najprv načíta `.env.example` baseline, potom `update(compose.environment)` (compose má precedence pre overlapping keys — `DB_HOST: db` v compose override-ne `DB_HOST=db` v example). Synthetic secret generation + DB rewrite + ${VAR} placeholder logic aplikuje na union dict — žiadne ďalšie zmeny. |
| **Synthetic secret format heuristic per suffix (CR-027)** | Synthetic secret format závisí od key suffix-u: `_KEY` (encryption keys per crypto library convention) → `base64.b64encode(secrets.token_bytes(32))` (44 ASCII chars **standardná alphabet `+/`**, decoded length = 32 bytes — kompatibilné s Fernet/NaCl/cryptography + nex-inbox `CredsCipher` ktorý používa strict `base64.b64decode(..., validate=True)`). Iné secret suffixes (`_PASSWORD`, `_SECRET`, `_TOKEN`) → `secrets.token_hex(32)` (64 hex chars, never decoded, safe for password/token usage). Standard base64 (`+/`) zvolený nad urlsafe (`-_`) lebo strict `b64decode` decoders odmietajú urlsafe chars (Bug #13 zachytený smoke testom 2026-05-26), zatiaľ čo `urlsafe_b64decode` akceptuje obe alphabets (per Python docs) → max compatibility. nex-inbox `infra/crypto.py:CredsCipher` strict checkuje 32 bajtov po base64 decode; hex32 dáva 48 bajtov → `EmailCredsKeyError` (Bug #12 zachytený smoke testom 2026-05-26). Helper `_uat_lib._synthetic_secret(key)` zapuzdruje dispatch; per-project format overrides deferred to CR-028+ (no universal standard). |
| **External alembic init container (CR-028)** | Pre `alembic_strategy="external"` UAT compose template renderuje **alembic-init** service: image shared s backend (build alebo `image:` ref), `depends_on: postgres.service_healthy`, `command: ["python", "-m", "alembic", "upgrade", "head"]`, `restart: "no"`. Backend dostáva conditional `depends_on: alembic-init.service_completed_successfully` → schema existuje keď backend's lifespan opens DB pool. Inak backend ktorý queruje schema v lifespan (napr. nex-inbox `recover_in_flight_invoices` na `invoices` table) zlyhá s `UndefinedTableError` predtým než post-deploy alembic step uat-deploy.py stihne zbehnúť (Bug #14 zachytený smoke testom 2026-05-26). `uat-deploy.py` post-deploy alembic step pre `external` strategy nahradený diagnostic log message (alembic-init už ran počas `compose up`). `self-bootstrap` strategy ostáva (backend lifespan migrates), `skip` strategy ostáva (no migration logic). |
