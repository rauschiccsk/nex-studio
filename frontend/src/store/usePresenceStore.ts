/**
 * Director presence ("away") store — E6 Telegram presence toggle (CR-NS-038).
 *
 * Today the cockpit pings the Director on Telegram only when no Director holds a
 ...
 * ``awaiting_director`` / ``blocked`` event pings Telegram **even with the board
 ...
 * the Director is still away). It is reset to ``false`` on login (see
 * ``authStore.login``) so "away" never carries silently into a fresh session.
 *
 * Manual revert (Director-approved 2026-06-12): the Director toggles back to "at
