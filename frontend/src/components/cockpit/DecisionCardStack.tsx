// DecisionCardStack — the interactive consultation surface (CR-V2-041).
//
// When the build is blocked on a CONSULTATION (block_reason="decision_needed"), the AI Agent has turned a
// problem (the Auditor's upfront findings; later any blocker) into a queue of plain-language DECISIONS. This
// renders them ONE CARD AT A TIME with a recommended default, so a non-expert Manažér (Tibor/Nazar) resolves
// the build by clicking — never reading raw findings or writing a prompt. Acceptance criteria (Director,
// 2026-06-29, design §6.1): (1) a clear "⛔ build stojí — treba rozhodnutie" signal; (2) plain language
// (the agent produced it); (3) an explicit question + what each button does; (4) it reads the ACTUAL
// blocking message (the kind=consultation block), never a stale gate_report.
...
export function DecisionCardStack({ messages, onDecide, inFlight = false }: Props) {
  ...
  const canSubmit = !inFlight && (showFree ? freeText.trim().length > 0 : picked != null);
  ...
  return (
    <div className="flex-shrink-0 border-b border-[var(--color-border-default)]">
      ...
      <div className="flex items-center gap-2 border-l-4 border-l-[var(--color-status-error)] bg-[var(--color-state-error-bg)] px-4 py-2.5 text-sm font-semibold text-[var(--color-state-error-fg)]">
        ...
        <span>
          ⛔ Build stojí — treba tvoje rozhodnutie ({idx + 1}/{total})
        </span>
      </div>
        ...
        <div className="rounded-lg border border-[var(--color-border-default)] bg-[var(--color-canvas)] p-4">
          ...
          <div className="mt-3 flex items-center justify-between gap-3">
            ...
            <span className="text-[10px] text-[var(--color-text-muted)]">
              Vyber možnosť a potvrď. Keď rozhodneš všetky body, AI Agent sám zapracuje zmeny a pokračuje.
            </span>
            <button
              type="button"
              onClick={submit}
            ...
            >
              {inFlight ? "Posielam…" : idx + 1 < total ? "Rozhodnúť → ďalšie" : "Rozhodnúť → dokončiť"}
            </button>
          </div>
        </div>
      </div>
