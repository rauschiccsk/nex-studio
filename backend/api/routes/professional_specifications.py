...
async def generate_design_doc(
    ...
    if doc_type == "design":
        system_prompt = (
            ...
            "TABUĽKY V SECTION 5 — SELF-CHECK POVINNÝ:\n"
            "- Každý riadok tabuľky musí mať PRESNE rovnaký počet | znakov ako header riadok\n"
            "- Po vygenerovaní každej tabuľky SPOČÍTAJ pipe-znaky v každom riadku a over zhodu\n"
            "- Ak riadok obsahuje zlomený obsah (napr. '| turnover|' namiesto '| turnover_to |'),"
            " je to chyba — oprav pred pokračovaním na ďalšiu tabuľku\n"
