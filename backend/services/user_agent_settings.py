...
def upsert(
    ...
    *,
    ...
    """Insert or update the ``(user_id, agent_role)`` row with ``model`` + ``effort``.

    Caller commits. Validation of ``agent_role`` / ``model`` / ``effort`` happens at the API layer
    (path Literal + pydantic enums); the DB CHECK on ``agent_role`` is the last line of defence.
    """
    row = db.execute(
    ...
    if row is None:
        row = UserAgentSettings(user_id=user_id, agent_role=agent_role, model=model, effort=effort)
        db.add(row)
    else:
        row.model = model
        row.effort = effort
